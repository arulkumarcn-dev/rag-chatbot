...
public class ChatController : ControllerBase
{
    ...
    public async Task<ActionResult> GenerateQuiz([FromBody] QuizGenerationRequest request)
    {
        try
        {
            if (string.IsNullOrWhiteSpace(request.Topic))
            ...
            var quiz = await _chatService.GenerateQuizAsync(request.Topic, request.QuestionCount);
            return Ok(new { success = true, quiz });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error generating quiz");
            return StatusCode(500, new { success = false, message = $"Error generating quiz: {ex.Message}" });
        }
    }
}
